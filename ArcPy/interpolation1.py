...
var_labels = ['MEAN_', 'SDEV_', 'SKEW_', 'MX5P_', 'RATIO_']
...
# ground = var_labels[0] + str(1) + '.txt'
# output = var_labels[0] + str(1) + '.tif'
# covars = [var + str(1) + '.tif' for var in covar_labels[:-1]]
...
for io in map_io_data[48:]:
  ...
  arcpy.management.XYTableToPoint(
    in_table=featA,
    out_feature_class=os.path.join(gdbDIR, ground.strip('.txt')),
    x_field="x",
    y_field="y",
    z_field=None,
  ...
  with arcpy.EnvManager(outputCoordinateSystem='GEOGCS["GCS_WGS_1984",DATUM["D_WGS_1984",SPHEROID["WGS_1984",6378137.0,298.257223563]],PRIMEM["Greenwich",0.0],UNIT["Degree",0.0174532925199433]]', snapRaster=rasterD, parallelProcessingFactor="3", extent='-120.995833333333 30.004166666666 -102.004166666666 42.995833333333 GEOGCS["GCS_WGS_1984",DATUM["D_WGS_1984",SPHEROID["WGS_1984",6378137.0,298.257223563]],PRIMEM["Greenwich",0.0],UNIT["Degree",0.0174532925199433]]', cellSize=rasterD):
      arcpy.ga.GALayerToGrid(
          in_geostat_layer='EBK Regression Prediction',
          out_surface_grid=os.path.join(outptDIR, ground.strip('.txt') + '.tif'),
...
# print(arcpy.Describe(os.path.join(dataDIR, 'GAoutput.lyr')))

# arcpy.SaveToLayerFile_management( 
#   in_layer=os.path.join(dataDIR, 'GAoutput.lyr'), 
#   out_layer=os.path.join(dataDIR, 'GAoutput.lyr')
# )
...
#   in_geostat_layer=os.path.join(dataDIR, 'GAoutput.lyr'),
#   out_raster=os.path.join(outptDIR, featFILE.strip('.txt') + '.tif'), 
